        ...
        try:
          ...
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("skip"), filters=filter, max_size=max_size):
                ...
                if forward_tag:
                   ...
                   new_caption = custom_caption(message, caption, replace_rules)  # Pass replace_rules
...
def custom_caption(msg, caption, replace_rules=None):  # Updated to accept replace_rules
  if msg.media:
    if (msg.video or msg.document or msg.audio or msg.photo):
      ...
      if media:
        ...
        if fcaption:
          ...
          if replace_rules:
            fcaption = apply_replace_rules(fcaption, replace_rules)
        
        if caption:
          formatted_caption = caption.format(filename=file_name, size=get_size(file_size), caption=fcaption)
          # Apply replacement rules to the custom caption too
          if replace_rules:
            formatted_caption = apply_replace_rules(formatted_caption, replace_rules)
          return formatted_caption
        
        # Apply replacement rules to file name in custom caption if no caption template
        if replace_rules and file_name:
          file_name = apply_replace_rules(file_name, replace_rules)
